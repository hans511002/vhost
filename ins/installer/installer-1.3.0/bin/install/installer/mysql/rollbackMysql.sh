...
if [ "$sqlFiles" = "" ] ; then
 echo "no SQL files found, rollback skiped."
exit 0
fi

echo "mysql data initing!"
 ...
 sqlDIR=`ls $bin -F |grep /|grep -v init |awk -F/ '{print $1}' | grep '^[0-9]'|sort -Vr`  # ÄæÐò
  ...
  echo "needUpdateDir=$needUpdateDir"
	for dir in $needUpdateDir ; do
			...
			sqlFiles=`ls   $bin/$dir/rollback/*.sql  2>/dev/null`
			
			if [ "$sqlFiles" = "" ] ; then
				 echo "version $dir not exists sql file in  $bin/$dir/rollback "
				 continue
			fi
			for sqlFile in $bin/$dir/rollback/*.sql ; do 
...
echo " $appName  mysql data  Installed!"
