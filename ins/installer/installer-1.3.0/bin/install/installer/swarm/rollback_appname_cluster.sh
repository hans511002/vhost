...
echo "docker tag $REGISTRY_DOMAIN:5000/${APP_NAME}:$_FROM_VERSION $REGISTRY_DOMAIN:5000/${APP_NAME}:latest"
docker tag $REGISTRY_DOMAIN:5000/${APP_NAME}:$_FROM_VERSION $REGISTRY_DOMAIN:5000/${APP_NAME}:latest
...
for HOST in `cat $${APPNAME}_HOME/conf/.upservers`; do
...
rm -rf $${APPNAME}_HOME/conf/.upservers
exit 0
