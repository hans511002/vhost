...
chmod -R 755 ${_CONF_DIR}  # 所有cnf不能赋权为777，否则会导致mysql因为安全策略的原因被忽略。
...
_DOCKER_TAG_NAME="${appName}-`hostname`"

echo "#!/bin/bash 
. /etc/bashrc
. \${APP_BASE}/install/funs.sh
checkRunUser ${appName}
 docker run --name=${appName}-\`hostname\` --net=host  -e VHOSTNAME=\`hostname\` \$DOCKER_OTHER_PARAMS \$${APPNAME}_RESOURCES  --privileged=true \
...
chmod +x $installFile
oldMysql=`docker ps -a|grep "$_DOCKER_TAG_NAME"|wc -l `
if [ "$oldMysql" -gt "0" ] ; then
    docker stop   $_DOCKER_TAG_NAME
    docker rm -f $_DOCKER_TAG_NAME
fi
...
RETRY_NUM=0
while [ 1 ] ;  do
    if [ -z "`docker ps | grep $_DOCKER_TAG_NAME`" ];then
        echo "$_DOCKER_TAG_NAME not running"
        ((RETRY_RUN_NUM++))
        if [ "$RETRY_RUN_NUM" -lt "2" ] ; then
            docker start $_DOCKER_TAG_NAME
        else
            exit 1
        fi
    fi
    echo "Try $RETRY_NUM times : docker exec ${_DOCKER_TAG_NAME} $MYCLI_NAME  -N -e \"show status  WHERE variable_name  LIKE 'wsrep_ready' OR variable_name LIKE 'wsrep_cluster_size' \""
    ALL_OUT=$(docker exec ${_DOCKER_TAG_NAME} $MYCLI_NAME  -N -e "show status  WHERE variable_name  LIKE 'wsrep_ready' OR variable_name LIKE 'wsrep_cluster_size' " 2>/dev/null)
    if [ "$ALL_OUT" != "" ] ; then
